...
public class BorrowingSystemValidator : Validator<BorrowingOperation>
{
    protected override void InitializeValidations()
    {
        ...
        Validations.Add("Book must be available for borrowing.", operation => operation is BorrowBook && operation.GetBook().BorrowingInfo.IsAvailable());
        ...
        Validations.Add("Book must not be already returned.", operation => operation is ReturnBook && !operation.GetBook().BorrowingInfo.IsAvailable());
    }
}